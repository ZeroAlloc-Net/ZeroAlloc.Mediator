...
```xml
<PackageReference Include="ZMediator" Version="0.1.0" />
<PackageReference Include="ZMediator.Generator" Version="0.1.0" OutputItemType="Analyzer" ReferenceOutputAssembly="false" />
```
...
The generator emits an `IZMediator` interface and `ZMediatorService` class with strongly-typed overloads that delegate to the static `Mediator`. This gives you constructor injection and testability with near-zero overhead (one virtual call — the JIT can often devirtualize):
...
// Injection
public class OrderController(IZMediator mediator)
{
...
| ZM003 | Warning | Request type is a class — use `readonly record struct` |
...
| ZM007 | Error | Stream handler returns wrong type instead of `IAsyncEnumerable` |

## Benchmarks

### ZMediator vs MediatR
...
|------------------------- |------------- |------------:|-------:|----------:|------------:|
| ZMediator_Publish_Single | Publish1     |   5.907 ns  |   1.06 |       0 B |          NA |
| MediatR_Publish_Single   | Publish1     | 221.578 ns  |  39.61 |     792 B |          NA |
|                          |              |             |        |           |             |
| ZMediator_Publish_Multi  | Publish2     |   5.273 ns  |   1.01 |       0 B |          NA |
| MediatR_Publish_Multi    | Publish2     | 299.262 ns  |  57.41 |   1,032 B |          NA |
|                          |              |             |        |           |             |
| ZMediator_Send           | Send         |   1.883 ns  |   1.62 |       0 B |          NA |
| MediatR_Send             | Send         |  75.159 ns  |  64.69 |     224 B |          NA |
|                          |              |             |        |           |             |
| ZMediator_Send_Static    | SendDI       |   2.539 ns  |   1.29 |       0 B |          NA |
| ZMediator_Send_DI        | SendDI       |   1.339 ns  |   0.68 |       0 B |          NA |
| MediatR_Send_DI          | SendDI       |  88.618 ns  |  44.90 |     224 B |          NA |
|                          |              |             |        |           |             |
| ZMediator_SendPipeline   | SendPipeline |   2.961 ns  |   1.14 |       0 B |          NA |
| MediatR_SendPipeline     | SendPipeline |  76.742 ns  |  29.51 |     152 B |          NA |
|                          |              |             |        |           |             |
| ZMediator_Stream         | Stream       | 149.316 ns  |   1.02 |     104 B |        1.00 |
| MediatR_Stream           | Stream       | 449.751 ns  |   3.06 |     528 B |        5.08 |
```

ZMediator is **26-65x faster** than MediatR with **zero allocation** on all synchronous paths. The DI interface (`IZMediator`) adds no measurable overhead vs the static API — both complete in ~1-3 ns with 0 bytes allocated. MediatR allocates 152-1,032 bytes per call due to DI resolution, delegate creation, and `Task<T>` boxing.
